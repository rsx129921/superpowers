...
| Pattern | Episodic TP | Episodic FP | Episodic AMB | Theoretical FP (of 3) | Recommendation | Notes |
|---------|-------------|-------------|--------------|----------------------|----------------|-------|
| `add tests` | 0 | 0 | 0 | 0 (of 3) | keep | Episodic: no verbatim hits (semantic search returned unrelated writing-plans results). Theoretical FPs tested: "should I add tests to this list?" (matched — marginal; in a dev session this leans TP), "who is going to add tests of the new metric to the dashboard" (matched — ambiguous; "tests of a metric" could mean coverage, still dev-context), "can you add tests for the edge cases we discussed" (matched — clear TP, not a FP at all). No unambiguous false positive found. Pattern is a tight imperative phrase; in a developer tool session it overwhelmingly means test coverage. Keep. |
| `tdd` | 0 | 0 | 0 | 3 (of 3) | keep | Episodic: no verbatim hits (low-quality matches, negative similarity scores). Theoretical FPs: "tdd is mentioned in the slack thread" (matched), "I disagree with the strict tdd interpretation" (matched), "the article on tdd was really interesting" (matched). All 3 matched, and technically these are meta-discussion rather than doing TDD. However, `tdd` is an acronym used almost exclusively as the topic word in dev tool sessions — a developer mentioning it in any context is highly likely to benefit from the skill prompt. Meta-discussion about TDD philosophy is closely related to TDD practice. FPs feel contrived outside a dev context. Keep; the co-occurrence cost is low and the signal value is high. |
| `test first` | 0 | 0 | 0 | 2 (of 3) | keep | Episodic: no verbatim hits (single low-quality result unrelated). Theoretical FPs: "lets test first whether the user wants this" (matched — UX/product validation, not code tests; genuine FP), "in the test, first I check if the value is null" (NO MATCH — word order "test, first" vs "test first" prevented match; good), "we should test first before rolling out to production" (matched — ambiguous; could mean test-the-code before deploy, which is a near-TP). 1 clear FP (product validation phrasing), 1 non-match on word-order variant (pattern behaves correctly), 1 TP-adjacent. Below the ≥2 unambiguous FP threshold for removal/tightening. Keep. |
| `write tests` | 0 | 0 | 0 | 1 (of 3) | keep | Episodic: no verbatim hits (negative similarity scores). Theoretical FPs: "the article said to write tests early" (matched — meta-discussion, but still a strong dev-context TP signal; borderline), "how do you write tests for legacy code conceptually" (matched — asking about testing methodology; borderline TP), "write tests of attention before deciding" (matched — psychology/neuroscience phrase "tests of attention", clear FP). Only 1 unambiguous FP ("tests of attention" is a domain-specific phrase unlikely in a dev session). The other two are borderline-TP at worst. Well below the ≥2 threshold. Keep. |
